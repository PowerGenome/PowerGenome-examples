...
for timeseries, downsampled to a shared 240-hour index (weather year 2012;
240 hours evenly spaced across the year, ~1 sample every 36.5 hours). All
files are CSV (or the small heat-rate/cpi tables below) and each is well
under a few MB.
...
| `reeds_load_tx_3_zone.csv` | Hourly load, 7 BAs × 240 h × 2 model years (3,360 rows) |
...
| `distributed_profiles_tx_3_zone.csv` | Distributed PV hourly profiles, 7 BAs × 240 h (1,680 rows) |
...
- **Regions**: keep only rows whose region column ∈ {p60, p61, p62, p63, p64,
  p65, p67}.
- **Timeseries**: keep only the shared 240-hour subset (every ~36.5th hour of
  the year, weather year 2012) used by every profile
